...
上手编译器常见的阻碍，不是“写不出代码”，而是“不知道从哪问起”。为了降低门槛，本章用可运行的极简语言 LXG，带你从零搭起“源码 → 字节码 → 执行”的主线，并在每一步给出能跑的命令、可视的产物与对照参照物（AST 文本、`javap -v` 反汇编）。
...
- 下一步：进入第 2 章，按“命令行 → 解析 → AST → 语义 → 生成 → 运行”的顺序加深理解；随后在语法与生成两端同时做一次小扩展（例如 `%`）。 